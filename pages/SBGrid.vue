...
export default {
  ...
  mounted: function () {
    if (typeof window !== 'undefined') {
      ...
      if (this.bind != null) {
        // SBGrid 속성 설정 중, bind 라는 속성의 값이 null이 아닐 경우, 본 컴포넌트에 이벤트를 할당합니다.
        for (let i = 0; i < Object.keys(this.bind).length; i++) {
          // 할당할 이벤트의 Key의 개수만큼 for문을 돌려 SBGrid 컴포넌트에 할당합니다.
          this.gridObject.bind(
            Object.keys(this.bind)[i],
            Object.values(this.bind)[i]
          ) // this.gridObject는 23번째 줄에 gridObject에 할당한 객체이며, bind() 메소드의 파라미터는 아래를 참조해주세요.
          /**
